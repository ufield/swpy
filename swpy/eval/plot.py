...
def create_observe_data(dst_df, omni_df, event_dt):
    ...
    # plot Dst --------
    dst_df.plot('Time', y='DST[nT]', color='black', ax=ax[0])

    ax[0].set_ylabel('DST[nT]')
    ax[0].xaxis.set_visible(False)

    # plot Bz & |B| ---
    omni_df.plot('Time', y=['F', 'BZ_GSM'], color=['black', 'red'], ax=ax[1])
    ax[1].legend(['|B|', 'Bz_GSM'])
    ax[1].set_ylabel('nT')
    ax[1].xaxis.set_visible(False)

    # plot Flow speed
    omni_df.plot('Time', y=['flow_speed'], color='black', ax=ax[2])
    ax[2].get_legend().remove()
    ax[2].set_ylabel('V [km/s]')
    ax[2].xaxis.set_visible(False)

    # plot proton_density
    omni_df.plot('Time', y=['proton_density'], color='black', ax=ax[3])
...
def create_plt_nn_result(result_dst_df, result_omni_df, event_dt):
    fig, ax = plt.subplots(4, 1, figsize=(14,12))
    ...
    ax[0].set_ylabel('DST[nT]')
    ax[0].legend(['Observed', '4 hours ahead prediction'])
    ax[0].xaxis.set_visible(False)
